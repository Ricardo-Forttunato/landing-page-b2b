        ...
        <div className="mt-12 grid gap-6 md:grid-cols-2 xl:grid-cols-4">
          {items.map((item, index) => {
            ...
            return (
              <article
                key={item.title}
                className="overflow-hidden rounded-sm border border-white bg-white shadow-sm"
              >
                <div
                  className={`flex h-44 items-end justify-between bg-gradient-to-br ${item.accent} p-4`}
                >
                  <img
                    src={imageByIndex}
                    alt={item.alt}
                ...
                <div className="space-y-2 p-6">
                  <p className="text-sm font-medium uppercase tracking-[0.2em] text-brand-gray">
                    {item.metric}
                  </p>
